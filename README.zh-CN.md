...
DSH-Themes Skills 是一组职责明确的安全工作流，不是主题注册表，也不对目录内容作笼统背书。Agent 可以查询用户信任的实时目录、创建纯数据清单、将清单交给网站审核，并且只管理完整本地门槛已经通过的制品。网站当前快照包含 97 条已发布记录：21 个 Themes、49 个 Skins、27 个 UI Extensions；按分发边界分为 32 个站内 V3 制品、11 个由独立规则约束的外部运行时条目和 54 个外部来源发现记录。Skin 目录中包含 26 套 Full Skin。这些实时数量描述网站内容，不会扩展任何本地可执行权威。
...
| **站内制品发布记录** | 一个精确的 `@dsh-themes/*` tarball、完整 SHA-256、受控同源下载路由，以及固定的 RC.8 兼容对象。Manager 经复核的当前映射固定 32 个精确的软件包—版本—摘要元组。 | 发布者身份、作者身份、素材权利、缺少独立绑定 release-set 证据的软件包级运行结果，或其他版本的安装权限。 |
...
- 站内主题的 **release-set 报告**回答某一批具体主题制品是否通过该次发布的构建、安装、重启、截图、回滚与摘要检查。`full-skins-2026-08` 现在将全部 26 套 Full Skin 记录为 `certified-rc8`。原始 20/20 矩阵、后续 4/4 刷新包抽查、定向 4/4 矩阵以及 Fire Horse Chronicle 的两阶段 1/1 最终验证分别绑定其实际检查的精确字节。定向矩阵中的 Fire Horse 字节后来被替换；最终 tarball 先通过用于生成最终索引的索引前抽查，再通过基于最终索引的冻结抽查，后者仅作为文档证据，并被明确排除在索引构建输入之外。对应计划、回执、归档、校验和与冻结身份摘要均列在[兼容性说明](skills/dsh-theme-manager/references/compatibility.md)中。这是软件包级发布证据，不是新的 Manager 认证，也不能代替固定的 RC.8 sidecar 或 attestation。

新的站内主题 slug 会在查询时从实时目录发现，但发现本身永远不会授予执行权限。Manager 当前的发布权威是从最终索引 SHA-256 `54686ba2df528b994840ab5ad33b24f06037e7bcdc0d3a034b0fb361456b658a` 生成并复核的 32 条软件包—版本—完整摘要记录；新的站内发布只有在最终制品产生后，才能通过经过复核的 Skills 发布进入这组权威。当前映射的声明顺序摘要为 `e3ed309a499b162041bff48e89f522b04866f612c504614fe80f5f56f2226e5c`。另有 24 个精确的 V1、V2、V3 前序版本位于独立的仅回滚映射中。全新安装与普通目录验证会拒绝全部 24 个版本；schema-2 回滚或反向恢复还必须让保留的发布记录、本地制品、版本、完整摘要与 payload 摘要完全一致。社区通道则不同：它精确的 11 条记录会一直保持本地固定，除非经过单独复核和重新认证。
...
- **站内主题：**Finder 对实时记录分类；Manager 根据本地 32 条当前权威再次验证精确 V3 发布，只通过受控路由下载，创建制品快照并保留回滚能力。退役制品不会进入这条普通路径。
...
- 站内映射包含 32 个当前可安装元组。另有 24 个保留的前序版本仅供回滚；它们必须同时匹配精确的 schema-2 记录与保留的旧发布记录。仅仅出现在 runner 内部摘要并集中，并不会形成独立安装授权。普通验证会继续把其中 6 个 V1/RC.5 与 13 个 V2/RC.6 软件包视为历史记录；只有经过复核的精确元组能够穿过狭窄回滚门槛。另 5 个 V3/RC.8 `1.1.0` 软件包遵循同一门槛。
...
- 站内软件包权威：**32 个当前可安装 V3 制品**，另有 **24 个保留的 V1/V2/V3 前序版本，仅供经过验证的回滚/反向恢复使用**。包含 26 套 Full Skin 的 `full-skins-2026-08` 软件包 release-set 已为 `certified-rc8`；上文给出了其独立证据摘要，完整边界见 [兼容性说明](skills/dsh-theme-manager/references/compatibility.md)。
