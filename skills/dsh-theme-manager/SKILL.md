...
description: Safely install, switch, remove, verify, or roll back verified DSH-Themes packages in the DeepSeek Harness web profile. Use for exact .tgz theme installation, active-theme inspection, Full Skin V3 compatibility checks, fail-closed V4 candidate inspection, conflict detection, built-in palette restoration, or rollback after a failed or unwanted theme change.
---
...
Baseline selection is policy-driven. Read `references/baseline-policy.json`; derive exact versions only from the pinned sidecars it names, and reject mutable dist-tags such as `latest` or `next`, ranges, and mixed evidence. Exact DeepSeek Harness `0.1.2-alpha.2` is now the separately promoted **operational Harness baseline**: run `33463453889` completed all six Linux/macOS/Windows × Node 22.19/24.15 tasks. That baseline deliberately grants no Theme, Skin, or Plugin item authority. Its separate 54-item V4 **candidate** projection still has 0/6 V4 runtime jobs and no byte-bound promotion receipt. Inspect it with `node scripts/alpha2-v4-candidate.mjs inspect`; require `installable: false` and `promotionAllowed: false`. The retained RC.8 lane remains the only operational item-install lane. Historical DeepSeek Harness `0.1.1-rc.2` also has a separately verified runtime baseline and detached Sigstore provenance; inspect it with `scripts/validate-promoted-rc2-runtime-baseline.mjs`, but never use it as alternate alpha.2 or item authority. Historical inputs remain immutable evidence.
...
2. Confirm that official DSH has already completed its own first start and that the selected profile exists. If not, stop and hand off to the separate `dsh-harness-installer` alpha.2 flow. Do not run Harness setup from Manager and do not combine it with a card-number installation.
...
- Never pass an alpha.2/V4 candidate digest to `run-dsh.mjs`, `fetch-and-verify.mjs`, or `validate-release.mjs`. V4 validation reports structure and expected bytes only; the promoted Harness baseline does not authorize installation or promotion while the V4 candidate's own six-task receipt set and byte-bound promotion receipt are absent.
...
Schema `4.0` is separately recognized only by `alpha2-v4-candidate.mjs`. A structurally valid V4 record is still non-installable. The current V3 validator and runner do not consume it.
