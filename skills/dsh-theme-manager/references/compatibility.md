...
The local hosted authority lives in [`scripts/hosted-artifact-authority.mjs`](../scripts/hosted-artifact-authority.mjs). `CURRENT_INSTALLABLE_HOSTED_ARTIFACTS` is generated from promoted `public/theme-packages/index.json` SHA-256 `0dd86b35ed13557d8dfa80b20a2290b17476fb03dc096b6f56bf4667c2377645` and binds 30 exact package-version-complete-digest tuples. The declared-order JSON entry array hashes to `a5ecde8b622c0a15e4d2aced53ac58fc567dab033fd40af7c93cc920c57772dd`:
...
The `full-skins-2026-08` release-set currently records `runtimeMatrix: "pending-managed-cold-restart"`. The 30-entry digest authority identifies the exact published bytes the Manager can accept; it does not claim those package versions have completed the release-set's isolated cold-start matrix. That package-level evidence must remain pending until the matrix is actually completed. It cannot alter or replace the immutable RC.8 Manager attestation.
