...
Community authority is now separate from that hosted lane. [`community-authority.json`](community-authority.json) preserves the byte-identical 11-item RC.8 identity and receipt history, while [`community-alpha1-recertification.json`](community-alpha1-recertification.json), SHA-256 `9ecc86474cba557c445ae21b8e479aa3f1b55cb8b2768faa6ed73952cc7b1552`, is the current decision gate. It binds exact `dsh-v0.1.2-alpha.1@cd5ef8148158c3a752a658978873241fdf8e2bbc`, 11 required items, 0 completed items, and 0 installable records. The website and Finder must therefore expose all 11 only as `external-showcase`, `showcase-only`, and `verification-pending`.
...
Finder requires an exact canonical public `#NNNN`, `runtime.status: "runtime-verified"`, `compatibility.status: "verified"`, exact `compatibility.baseline: "0.1.0-rc.8"`, fixed source package/version/revision/subdirectory, non-hold rights, and a byte-for-byte match with `references/community-authority.json` before it will even recognize this as one of the old 11. It then consults the independent alpha.1 gate and deterministically emits `external-showcase`, `showcase-only`, `verification-pending`, `installable: false`, `installer: null`, and `alpha1-community-recertification-pending`. The record may not supply an artifact URL or install command. No canonical ID, old Manager evidence, or descriptive edit can restore a handoff.
