...
Inspect only records admitted by the bundled catalog. This is a separate trust lane from `dsh-theme-manager`: Manager handles hosted, declarative `@dsh-themes/*` artifacts, while this Skill describes a small allowlist of executable upstream packages and reviewed local asset adaptations. The checked-in public release is intentionally inspection-only because its adjacent Manager remains certified for RC.6 and every RC.8 community item is still `verification-pending`.

## Boundaries

- RC.8 is the exact certification target, not current installation authority. Reject rc.6/rc.7 community execution, ranges, mutable dist-tags, mixed runners, and any attempt to reuse the RC.6 Manager runner.
...
- QQ98 and Red Market Terminal are DSH-Themes CSS-only RC.8 adaptations. They omit legacy injected chrome, icons, title changes, RPC, and executable hooks; the result intentionally differs from the historical preview.
- The install gate is enforced in scripts as well as instructions: it requires both item-level `runtime-verified` authority and the bundled allowlist's exact RC.8 Manager-certification gate. Neither condition is true in this release. The repository `release-state.json` is informational and is not executable authority.

Read [references/compatibility.md](references/compatibility.md) before changing a profile. Read [references/catalog-contract.md](references/catalog-contract.md) only when validating or publishing a new catalog record.
...
The validator also reports the adjacent Manager release gate. Inspection is allowed while either gate is pending; installation is blocked. Do not edit the temporary record or bundled allowlist to bypass either gate.

## Inspect the RC.8 profile after Manager certification

The adjacent `dsh-theme-manager` must first publish an exact RC.8 V3 attested runner. The current public Manager is RC.6, so stop here in this release. After a future repository release atomically promotes the Manager and community records, follow its compatibility bootstrap and then require:
...
## Install after both gates open

This section is dormant in the current public release. Immediately before a future authorized mutation, re-run `validate-record.mjs --mode install`, summarize current profile state, exact source, executable/network behavior, rights restrictions, prepared artifact, and rollback target, and obtain confirmation. Then:
...
Quote every path. A pending record may be inspected or downloaded for byte verification, but it may not be installed. Do not use `~`, `$HOME`, an unresolved environment variable, a workspace root, `npx`, `pnpm dlx`, a PATH `dsh`, source lifecycle scripts, or browser calls to non-loopback Harness URLs.
